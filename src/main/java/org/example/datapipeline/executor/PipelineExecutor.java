...
public class PipelineExecutor {
    ...
    public static void execute(Job job) {
        ...
        for (int level = 0; level < levels.size(); level++) {
            ...
            levels.get(level)
                    .parallelStream()
                    .forEach(stage -> executeStage(stage, job.getDatasourceMap()));
        }
    }

    private static void executeStage(Stage stage, Map<String, Datasource> globals) {
        ...
        while (true) {
            try {
                for (Task task : stage.getTasks()) {
                    ...
                    if(task.getInput() != null) task.getInput().resolve(globals);
                    if(task.getOutput() != null) task.getOutput().resolve(globals);
